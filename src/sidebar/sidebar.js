...
const EXPLORERS = {
  ...
  async function init() {
    ...
    chrome.runtime.onMessage.addListener((m) => {
      if (m.type === "TIP_SENT") {
        showMsg("ok", `Tip sent: $${m.data.amount} → ${m.data.creatorUsername}`);
        refreshDashboard();
      }
      if (m.type === "HYPE_UPDATE") updateHype(m.data);
    });
  ...
  function switchTab(tab) {
    ...
    // If wallet not connected, always show onboarding on dashboard tab
    if (!walletReady && tab === "dashboard") {
  ...
  async function toggleAutoTip() {
    ...
    if (res.success) {
      ...
      showMsg("ok", autoTipOn ? "Auto-tip ON — polling every 5s" : "Auto-tip OFF");
    }
  }
  ...
  async function runCycle() {
    ...
    if (res.success) {
      ...
      const tipCount = d.tips?.length || 0;
      showMsg("ok", `Cycle done — Hype: ${d.hype?.score || 0}/100, Tips: ${tipCount}`);
      refreshDashboard();
    } else {
      showMsg("err", res.error || "Cycle failed");
    }
  }
      ...
      return `
        <div class="tip-item">
          <div class="tip-left">
            ...
            <div class="tip-info">
              <div class="tip-creator">${t.creatorUsername}</div>
              <div class="tip-reason">${(t.triggerReason || "").replace(/_/g, " ")}</div>
            </div>
          </div>
          <div class="tip-right">
